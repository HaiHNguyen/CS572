...
function handleRequest(reqResp){
    ...
    console.log("Method:", reqResp.req.method);
    if(reqResp.req.method === 'GET'){
        processGetRequest(reqResp);
    }else if(reqResp.req.method === 'POST' ){
        processPostRequest(reqResp);
    }else{
        reqResp.resp.end(reqResp.req.method + 'is not supported');
    }
}
...
function processGetRequest(reqResp){
    const url = new Url();
...
function processPostRequest(reqResp){

    getRequestBody(reqResp.req, (requestBody) =>{
